...
public class AvgScoreSinkToMongo extends RichSinkFunction<Tuple3<Long, Double, Long>> {
    ...
    @Override
    public void invoke(Tuple3<Long, Double, Long> value, Context context) throws Exception {
        UpdateOptions options = new UpdateOptions().upsert(true);
        collection.updateMany(Filters.eq("productId", value.f0),
                new Document("$set", new Document("productId", value.f0).append("score", value.f1)), options);
    ...
    private void buildConnection() {
        ...
        // 连接到数据库
        if (collection == null) {
            collection = mongoDatabase.getCollection("AverageScore");
        }
